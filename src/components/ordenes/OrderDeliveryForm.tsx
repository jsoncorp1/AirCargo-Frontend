...
export default function OrderDeliveryForm({ mode, orderId, onClose, onSaved }: OrderDeliveryFormProps) {
  ...
  // La empresa de la orden: define si se puede ofrecer "a cuenta".
  ...
  const { hasCreditAccount } = useSupplierCreditAccount(supplierId);
  ...
  useEffect(() => {
    if (paymentType === "OnAccount" && !hasCreditAccount && !readOnly) {
      setPaymentType("Prepaid");
    }
  }, [hasCreditAccount, paymentType, readOnly]);
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div>
                ...
                {paymentType === "OnAccount" && (
                  <p className="mt-1.5 text-xs text-gray-400 dark:text-gray-500">
                    El importe se suma al estado de cuenta del mes y se cobra al cierre.
                  </p>
                )}
              </div>
