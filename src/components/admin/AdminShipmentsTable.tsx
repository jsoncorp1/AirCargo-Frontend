...
import { EyeIcon, BoxCubeIcon, TaskIcon, CloseLineIcon } from "@/icons";
import {
...
export default function AdminShipmentsTable({
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedShipmentIds(shipments.filter(s => s.validity !== 'Annulled').map(s => s.id));
    } else {
                    ...
                    >
                      <TableCell className="px-5 py-4">
                        <input 
                          type="checkbox" 
                          disabled={isAnnulled}
                          className="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500 disabled:opacity-30 disabled:cursor-not-allowed"
                          checked={isSelected}
                      ...
                      <TableCell className="px-5 py-4">
                        <div className="flex flex-col items-start gap-1">
                          ...
                          {isAnnulled && shipment.annulmentReason && (
                            <span className="text-[11px] text-error-600 dark:text-error-400 italic max-w-[200px] truncate" title={shipment.annulmentReason}>
      ...
      >
        <div className="p-6">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
            Esta acción quema el correlativo y la guía deja de tener validez. Si atendía una orden de entrega, la orden volverá a quedar pendiente.
          </p>
