...
export default function UserDropdown() {
  ...
  const { user, companyName, branchOfficeLabel, isSupplierUser, logout } = useAuth();
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 flex items-center justify-center h-11 w-11 rounded-full bg-brand-500 text-white text-base font-bold select-none">
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-3 pb-3 border-b border-gray-100 dark:border-gray-800">
          <span className="flex items-center justify-center h-10 w-10 rounded-full bg-brand-500 text-white text-sm font-bold select-none flex-shrink-0">
            {getInitials(user?.fullName)}
          </span>
          <div className="min-w-0">
            ...
            {/* Un usuario de proveedor no opera sucursales; para el resto se
                muestra la suya, o "Todas las sucursales" si es global. */}
            {!isSupplierUser && (
              <span className="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400 truncate">
                Sucursal: {branchOfficeLabel}
              </span>
            )}
          </div>
        </div>
